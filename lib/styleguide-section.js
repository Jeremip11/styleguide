...
export default class StyleguideSection {
  ...
  render () {
    if (this.loaded) {
      let className = 'styleguide-Section'
      if (this.collapsed) {
        className += ' is-collapsed'
      }
      return (
        <section className={className} dataset={{name: this.name}}>
          <h1 className='styleguide-Section-title' onclick={() => this.toggle()}>{this.title}</h1>
          {this.children}
        </section>
      )
    } else {
      return (
        <section className='styleguide-Section is-collapsed' dataset={{name: this.name}}>
          <h1 className='styleguide-Section-title' onclick={() => this.toggle()}>{this.title}</h1>
        </section>
      )
    }
